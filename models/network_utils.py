...
class FusionMLP(nn.Module):
    def __init__(self, dim_in, dim_cond, dim_out, config, dim_coord=3):
        ...
        self.embed_fn = None
        if config.multires > 0:
            embed_fn, input_ch = get_embedder(config.multires, input_dims=dim_in + dim_coord)
            self.embed_fn = embed_fn
            dims[0] = input_ch
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l in config.cond_in:
                lin = nn.Linear(dims[l]+dims[l] + dim_cond, out_dim)
            else:
                lin = nn.Linear(dims[l], out_dim)
        ...
        # self.attention_fusion = SelfAttentionFusion(dim_coord, dim_in - dim_coord, config.n_neurons)

    def forward(self, coords, geometry_feats, cond=None):
        if cond is not None:
            cond = cond.expand(coords.shape[0], -1)
        if self.embed_fn is not None:
            coords_embedded = self.embed_fn(coords)
        ...
        # Concatenate coords and
        geometry_feats = geometry_feats.repeat((coords.shape[0] // geometry_feats.shape[0] + 1), 1)[:coords.shape[0], :]
        x = torch.cat([coords_embedded, geometry_feats], dim=1)
        #
        # x = self.attention_fusion(coords_embedded, geometry_feats)
