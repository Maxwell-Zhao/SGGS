...
class H36MDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        with open(os.path.join(self.root_dir, self.subject, 'Posing', 'cam_params.json'), 'r') as f:
        ...
        subject_dir = os.path.join(self.root_dir, self.subject, 'Posing')
        if split == 'predict':
        ...
        if split == 'predict' or cfg.freeview:
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                cam_dir = os.path.join(subject_dir, cam_name)
                # semantic_dir = os.path.join(subject_dir, self.subject)
                # semantic_files = sorted(glob.glob(os.path.join(semantic_dir, '*.png')))[frame_slice]
                ...
                for d_idx, f_idx in enumerate(frames):
                    ...
                    model_file = model_files[d_idx]
                    # semantic_file = semantic_files[d_idx]
    ...
    def getitem(self, idx, data_dict=None):
        ...
        # semantic_file = data_dict['semantic_file']
        ...
        # semantic = cv2.cvtColor(cv2.imread(semantic_file), cv2.COLOR_BGR2RGB)
        ...
        # semantic = torch.from_numpy(semantic).permute(2,0,1).float()
